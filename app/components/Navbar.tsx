...
export default function Navbar() {
  ...
  return <nav className="fixed inset-x-0 top-0 z-50 border-b border-border/80 bg-background/80 backdrop-blur-xl">
    <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5">
      <Link href="/" className="flex items-center gap-3 text-text"><Image src="/virtunex.png" alt="Virtunex" width={34} height={34} /><span className="text-xl tracking-wide">Virtunex</span></Link>
      <div className="hidden items-center gap-5 md:flex">{links.map(([label, href]) => <Link key={label} href={href} className="text-sm text-muted transition-colors hover:text-accent-light">{label}</Link>)}{user ? <Link href="/dashboard" aria-label="Profile"><Image src={user.profileImage || '/virtunex.png'} alt={user.fullName || 'Profile'} width={32} height={32} className="h-8 w-8 rounded-full border border-border object-cover" /></Link> : <Link href="/login" className="bg-jamoon-bright px-4 py-2 text-sm text-text hover:bg-accent">Get Started</Link>}</div>
